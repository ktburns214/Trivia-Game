...
var questionStore = [
    {
        q: "The butler, Carson, had a shameful secret that was revealed by an unwelcome visitor to Downton. What was it?",
        a: ["He is illegitimate", "He has jilted a woman at the alter", "He has performed in music halls", "He has been in the workhouse"],
        correct: 2
    },

    {
        q: "What relation was Matthew Crawley, heir to Downton Abbey, to Lady Mary, his wife?",
        a: ["Third cousin, once removed", "Second cousin", "Brother", "Fourth Cousin"],
        correct: 3
    },

    {
        q: "Vera Bates blackmailed her husband, John, for a chunk of his inheritance by threatening to reveal what information?",
        a: ["His crimial record for theft", "He has been unfaithful", "A scandal of Lady Mary and Kamal Pamuk"],
        correct: 2
    },
...

    {
        ...
        a: ["Duneagle Castle", "Inveraray Castle", "Eilean Donan Castle", "Craigievar Castle"],
        correct: 0
    },

    {
        q: "What was the name of Lord Robert’s previous valet who is replaced by Mr. Bates?",
        a: ["Watson", "Smith", "Holmes", "Williams"],
        correct: 0
    },

    {
        q: "The former prince from who tries to rekindle an old romance with the Dowager Countess is from Germany",
        a: ["True", "False"],
        correct: 1
    }
]
...
function checkWin() {
    if (bestScore < correctAnswers) {
        ...
        $(".results").text("You got a new high score of " + bestScore + "!  I wonder your halo doesn’t grow heavy. It must be like wearing a tiara around the clock.");
    } else {
        $(".results").text("All life is a series of problems that we must try and solve.  You got " + correctAnswers + " correct.  But there is still room for improvement if you want to beat the high score of " + bestScore + ".");
    }
    $(".question-area").empty();
    $(".answer-table").empty();
    var nextGame = $("<div>");
...
$(document).on("click", ".skip-bo", function () {
    if (hasAnswered === false) {
        ...
        $(".question-area").empty();
        $(".answer-table").empty();
        $(".question-area").text("Don’t be defeatist, dear, it’s very middle class.");
